...
module.exports = {
  addJobNoty : function(jobInstNew,adminInst,ln){
    ...
    return {
          ...
          title    : jobInstNew.category.name || "New Job",
  ...
  vendorAssigned : function(jobInstNew,vendorInst,ln){
    ...
    return {
          ...
          title    : jobInstNew.category.name,
          body     : "You got new Job please accept ASAP.",
  ...
  engineerAssigndCust : function(jobInstNew,custInst,ln){
   ...
   // notyDate =  new Date();
    ...
    return {
          //title    : "Engineer_Assigned",
           title    : jobInstNew.category.name,
          body     :  "Engineer has assigned for your job and your OTP is "+jobInstNew.otp,
          "Date"   : jobInstNew.updatedAt.toISOString(),
          //"Date"   : notyDate.toISOString(),
  ...
  startJobVendor : function(jobData,vendorInst,ln){
   ...
   // //console.log(newDate.getHours());
        ...
        return {
          ...
          title    : jobInstNew.category.name,
           body     : "Fixpapa engineer "+jobInstNew.engineer.fullName+" has scheduled your job at "+scheduleDate,
           "Date"   : jobInstNew.updatedAt.toISOString(),
          // "Date"   : notyDate.toISOString(),
   ...
   scheduleJobVen : function(jobInstNew,vendorInst,ln){
    ln = ln || "en";
  //notyDate =  new Date();
  let newDate = new Date(jobInstNew.schedule.eStartDate);
   // let scheduleDate = date.format(newDate, 'YYYY/MM/DD HH:mm:ss');
    ...
    return {
          // title    : "Engineer_Schedule_Job",
           title    : jobInstNew.category.name,
           body     : jobInstNew.engineer.fullName+" has scheduled job at "+scheduleDate,
          "Date"   : jobInstNew.updatedAt.toISOString(),
          //"Date"   : notyDate.toISOString(),
  ...
  pickProCust : function(jobInstNew,custInst,ln){
    ...
    return {
           //title    : "Pick_Product",
           title    : jobInstNew.category.name,
           body     : "We will deliver your product with in "+jobInstNew.pick.duration+" days .",
  ...
  requestPart : function(jobData,custInst,ln){
    ...
    return {
         // title    : "Part_Request",
          title    : jobData.category.name,
          body     : "Fixpapa engineer "+jobData.engineer.fullName+"has requested for additional parts.",
          "Date"   : jobData.updatedAt.toISOString(),
          //"Date"   : notyDate.toISOString(),
          "type"   : "part_request",
          "jobId"  : jobData.id.toString(),
  ...
  responsePart : function(jobData,engiInst,ln){
    ...
    return {
          //title    : "Part_Response",
          title    : jobData.category.name,
          body     : jobData.customer.fullName+" sent response of part request "+jobData.orderId,
          "Date"   : jobData.updatedAt.toISOString(),
          //"Date"   : notyDate.toISOString(),
          "type"   : "part_response",
          "jobId"  : jobData.id.toString(),
   ...
   // notyDate =  new Date();
    ...
    return {
          ...
          title    : jobInstNew.category.name,
          body     : "Your product "+jobInstNew.status,
         "Date"   : jobInstNew.updatedAt.toISOString(),
         //"Date"   : notyDate.toISOString(),
  ...
  generateBillCust : function(jobInstNew,custInst,ln){
    ...
    return {
          //title    : "Bill_Generate",
          title    : jobInstNew.category.name,
          body     : "Your Job has completed successfully please pay your bill.",
         "Date"   : jobInstNew.updatedAt.toISOString(),
         //"Date"   : notyDate.toISOString(),
          "type"   : "bill_generate",
          "jobId"  : jobInstNew.id.toString(),
  ...
  completeJobVen : function(jobInstNew,vendorInst,ln){
   ...
   cancelJobVendor : function(jobInstNew,vendorInst,ln){
    ...
    return {
          title    : jobInstNew.category.name,
          body     : jobInstNew.orderId+" job has  cancelled our executive will connect with you shortly.",
          "Date"   : jobInstNew.updatedAt.toISOString(),
          //"Date"   : notyDate.toISOString(),
    ...
    return {
          title    : jobInstNew.category.name,
          body     : jobInstNew.orderId+" job has  cancelled your vendor will connect with you shortly",
          "Date"   : jobInstNew.updatedAt.toISOString(),
          //"Date"   : notyDate.toISOString(),
          "type"   : "cancel_job_eng",
          "jobId"  : jobInstNew.id.toString(),
   ...
   paymentCash : function(jobInstNew,vendorInst,pDate,ln){
    ln = ln || "en";
     // notyDate =  new Date();
     jobInstNew.category = jobInstNew.category || {};
    ...
    return {
          title    : jobInstNew.category.name,
          body     : "Customer Paid cash for job "+jobInstNew.orderId,
   ...
   paymentCashCust : function(jobInstNew,custInst,pDate,ln){
    ln = ln || "en";
     // notyDate =  new Date();
     jobInstNew.category = jobInstNew.category || {};
    ...
    return {
          title    : jobInstNew.category.name,
          body     : "Thanks for the payment, Please provide your valuable feedback.",
          "Date"   : new Date().toISOString(),
          //"Date"   : notyDate.toISOString(),
  ...
  paymentCheque : function(jobInstNew,vendorInst,pDate,ln){
    ...
    return {
          title    : jobInstNew.category.name,
          body     : "Customer Paid by cheque for job "+jobInstNew.orderId,
  ...
  paymentChequeCust : function(jobInstNew,custInst,pDate,ln){
    ln = ln || "en";
     // notyDate =  new Date();
     jobInstNew.category = jobInstNew.category || {};
      pDate = new Date(pDate)|| new Date();
  ...
  paymentOnline : function(jobInstNew,vendorInst,pDate,ln){
    ...
    return {
          title    : jobInstNew.category.name,
          body     : "Customer Paid online for job "+jobInstNew.orderId,
  ...
  paymentOnlineCust : function(jobInstNew,custInst,pDate,ln){
    ln = ln || "en";
  ...
  autoVenCancel : function(jobInstNew,vendorInst,ln){
   ...
   cronCust1 : function(jobInstNew,custInst,ln){
    ...
    return {
          ...
          title    : jobInstNew.category.name,
          body     : "Your job has cancelled due to some technical reason , we will connect with you shortly.",
          "type"   : "notify_cust",
          "Date"   : notyDate.toISOString(),
          "jobId"  : jobInstNew.id.toString(),
          "badge"  : (custInst.newNotification || 0).toString(),
  ...
  cronCust2 : function(jobInstNew,custInst,ln){
    ...
    return {
          ...
          title    : jobInstNew.category.name,
          body     : "Your job has  cancelled our executive will connect with you shortly.",
          "type"   : "notify_cust2",
          "Date"   : notyDate.toISOString(),
          "jobId"  : jobInstNew.id.toString(),
          "badge"  : (custInst.newNotification || 0).toString(),
  ...
  loginNotyy : function(userInst,ln){
    ln = ln || "en";
    let notyDate = new Date();
    //jobInstNew.category = jobInstNew.category || {};
    return {
          ...
          //"content_available": "1",
          //"jobId"  : jobInstNew.id.toString(),
          //"badge"  : (userInst.newNotification || 0).toString(),
          "sound"  : "default",
